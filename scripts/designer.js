...
function saveDesign() {
  ...
  function setupEventListeners() {
    ...
    elements.zoomSlider.addEventListener('touchstart', function(e) {
    e.stopPropagation();
      }, { passive: true });

      elements.zoomSlider.addEventListener('touchmove', function(e) {
          e.stopPropagation();
          state.zoom = parseFloat(this.value) / 100;
          updateZoom();
      }, { passive: true });

      elements.zoomSlider.addEventListener('touchend', function(e) {
          e.stopPropagation();
      }, { passive: true });
    ...
    document.addEventListener('keydown', function(e) {
        // Отключаем горячие клавиши на мобильных устройствах и когда фокус в поле ввода
        if (state.isMobile || document.activeElement === elements.engravingText) return;
